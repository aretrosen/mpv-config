					...
					if mouse_hit(element) then
						...
						if element.thumbnailable and not thumbfast.disabled then
							...
							if osd_w then
								...
								local se, ae = state.slider_element, elements[state.active_element]
								if
									user_opts.chapterformat ~= "no"
									and se
									and (ae == se or (not ae and mouse_hit(se)))
								then
									local dur = mp.get_property_number("duration", 0)
									if dur > 0 then
										local possec = get_slider_value(se) * dur / 100 -- of mouse pos
	...
	local ret = mp.command_native_async({
		...
		if state.youtubeuploader then
			state.localDescriptionClick = state.localDescriptionClick:gsub(
				"Uploaded by: <$\\N!uploader!\\N$>",
				"Uploaded by: " .. state.youtubeuploader
			)
		else
			state.localDescriptionClick =
				state.localDescriptionClick:gsub("Uploaded by: <$\\N!uploader!\\N$>", "Uploaded by: ")
		end
		...
		if not state.videoCantBeDownloaded then
			...
			local function openFolder()
				local function is_macos()
					...
					if a ~= nil and string.sub(a, 1, 6) == "/Users" then
						return true
					else
						return false
					end
